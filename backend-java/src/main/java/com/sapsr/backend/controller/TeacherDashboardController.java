...
public class TeacherDashboardController {
    ...
    public ResponseEntity<?> getSubmissions(HttpServletRequest request) {
        ...
        List<Submission> submissions = submissionRepository
                .findByTeacher_TelegramIdAndStatusOrderByCreatedAtDesc(telegramId, "SUCCESS");
        ...
        for (Submission s : submissions) {
            ...
            if (s.getStudent() != null) {
                item.put("student_name", s.getStudent().getFullName());
            }
    ...
    public ResponseEntity<?> setVerdict(@PathVariable Integer id,
        ...
        if (submission.getStudent() != null) {
            Long studentChatId = submission.getStudent().getTelegramId();
            String teacherName = teacher.get().getFullName() != null ? teacher.get().getFullName() : "Преподаватель";
            String text;
            if ("APPROVED".equals(verdict)) {
                text = "✅ Преподаватель " + teacherName + " принял вашу работу!";
            } else {
                text = "🔄 Преподаватель " + teacherName + " отправил работу на доработку.\n\nКомментарий: " + comment;
            }
            bot.notifyUser(studentChatId, text);
        }
